...
function TitanDarknessController:init()
    ...
    for i = 1, 40 do
        table.insert(self.fumes,
            { -5 + i * 25, 330, MathUtils.random(20, 40), self.timer +
            MathUtils.random(-30, 30), false })
    end

    for i = 1, 40 do
        table.insert(self.fumes,
            { -5 + i * 25, -10, MathUtils.random(20, 40), self.timer +
            MathUtils.random(-30, 30), false })
    end
    ...
    for i = 1, 40 do
        table.insert(self.fumes,
            { 655, -5 + i * 25, MathUtils.random(20, 40), self.timer +
            MathUtils.random(-30, 30), false })
    end
...
function TitanDarknessController:update()
    ...
    if self.toggle_lessen then
        self.alpha_gain = MathUtils.approach(self.alpha_gain, 0, 0.03 * DTMULT)
    else
    ...
    if self.spawn_timer < 0 then
        ...
        table.insert(self.fumes,
            { MathUtils.random(0, SCREEN_WIDTH), SCREEN_HEIGHT + 30, MathUtils.random(10, 40), self.timer, true })
    end
...
function TitanDarknessController:getFumeInformation(index)
    local x, y, radius, time, shrink_move = TableUtils.unpack(self.fumes[index])
    if shrink_move then
